...
begin
  ...
  if coalesce(v_booking.resource_type, 'asset'::public.borrowing_resource_type) <> 'asset'::public.borrowing_resource_type then
  ...
  where b.status in ('pending'::public.booking_status, 'approved'::public.booking_status, 'checked_out'::public.booking_status)
    ...
    and (
      ...
      or (
        ...
        and ba.location_id = v_asset.location_id
        and ba.is_room_bound = true
        and v_asset.is_room_bound = true
      )
    )
